document.addEventListener('DOMContentLoaded', () => {
  ...
  const el = document.getElementById("proxyStatusText");
  ...
  const textEl = row.querySelector(".copy-text");
  const text = textEl.dataset.copy || textEl.textContent;
  ...
  function handleDonateBtn() {
    elements.donateDropdown.classList.toggle("hidden");
  }
  ...
  function handleCopyText(e) {
    ...
    const textEl = document.getElementById("donateText");
  ...
  data[tabId].domains =
    data[tabId].domains.filter(d => d !== domain);
